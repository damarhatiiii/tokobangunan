...
<body>
    <h1>Toko Bangunan</h1>
    <p class="muted">Struk penjualan &middot; #<?= (int) $row['id_keluar'] ?><br><?= esc($row['tanggal']) ?></p>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td colspan="2" style="padding-top:8px;font-size:11px;color:#555;">Harga satuan: Rp <?= esc($harga_print) ?></td>
            </tr>
        </tbody>
    </table>
    ...
    <?php if (! empty($row['nama_kasir'])): ?>
        <p style="margin-top:1rem;font-size:11px;color:#666;">Kasir: <?= esc($row['nama_kasir']) ?></p>
    <?php endif; ?>

    <div class="actions no-print">
        <button type="button" onclick="window.print()">Cetak</button>
        <a href="<?= base_url('sales') ?>">Kembali ke daftar</a>
    </div>
</body>
</html>
