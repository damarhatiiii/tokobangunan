...
<div class="mb-6 flex flex-wrap items-center justify-between gap-4">
    <h1 class="text-2xl font-semibold text-neutral-900 dark:text-white">Kategori barang</h1>
...
<form method="get" class="mb-4 flex max-w-md gap-2">
    <input type="search" name="q" value="<?= esc($q) ?>" placeholder="Cari nama…" class="flex-1 rounded-lg border border-neutral-300 px-3 py-2 text-sm dark:border-neutral-600 dark:bg-neutral-800 dark:text-white">
            ...
            <?php foreach ($rows as $row): ?>
                <tr>
                    ...
                    <td class="px-4 py-3 text-right space-x-2">
                        ...
                        <form method="post" action="<?= base_url('categories/delete/' . $row['id_kategori']) ?>" class="inline" onsubmit="return confirm('Hapus kategori ini?')">
                            <?= csrf_field() ?>
